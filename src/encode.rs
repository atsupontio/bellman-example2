...
pub fn create_uncompressed_file(res_proof: String, res_vkey: String) -> Result<(), io::Error> {

    println!("... creating uncompressed files");

    let mut file_uncompressed_proof = File::create("./file/proof_uncompressed.json").unwrap();
    file_uncompressed_proof.write(res_proof.as_bytes())?;

    let mut file_uncompressed_vkey = File::create("./file/vkey_uncompressed.json").unwrap();
    file_uncompressed_vkey.write(res_vkey.as_bytes())?;

    Ok(())
}

pub fn create_multi_uncompressed_files(res_proof: String, res_vkey: String, count: u32) -> Result<(), io::Error> {

    for i in 1..=count {
        let mut file_uncompressed_proof = File::create(format!("{}{}{}", "./batch_file/proof", i, "_uncompressed.json")).unwrap();
        file_uncompressed_proof.write(res_proof.as_bytes())?;

        let mut file_uncompressed_vkey = File::create(format!("{}{}{}", "./batch_file/vkey", i, "_uncompressed.json")).unwrap();
        file_uncompressed_vkey.write(res_vkey.as_bytes())?;
    }
...
pub fn encode_uncompressed_1input() -> Result<(), io::Error> {
