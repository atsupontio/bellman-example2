...
pub fn read_scalar<R: Read>(mut reader: R) -> io::Result<jubjub::Fr> {
...
pub fn h_star(a: &[u8], b: &[u8]) -> Option<jubjub::Fr> {
    hash_to_scalar(b"Zcash_RedJubjubH", a, b).into()
}
...
impl PrivateKey {
    ...
    pub fn sign<R: RngCore>(&self, msg: &[u8], rng: &mut R, p_g: SubgroupPoint) -> Signature {
        ...
        // S = r + H*(Rbar || M) . sk
        let mut s = h_star(&rbar[..], msg).unwrap();
        s.mul_assign(&self.0);
        s.add_assign(&r);
        let mut sbar = [0u8; 32];
...
impl PublicKey {
    ...
    pub fn verify_with_zip216(
        ...
        // c = H*(Rbar || M)
        let c = h_star(&sig.rbar[..], msg).unwrap();
